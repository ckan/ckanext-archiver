...
from ckanext.archiver.tasks import (
    link_checker,
    update_resource,
    update_package,
    download,
...
class TestLinkChecker:
    ...
    def test_url_with_30x_follows_redirect(self, client):
        redirect_url = client + "/?status=200&content=test&content-type=text/csv"
        url = client + "/?status=301&location=%s" % quote_plus(redirect_url)
        context = json.dumps({})
        data = json.dumps({"url": url})
...
class TestArchiver:
    ...
    def test_file_too_large_1(self, client):
        ...
        assert not result, result
        self.assert_archival_error(
            "Content-length 1000001 exceeds maximum allowed value 1000000", res_id
        )

    def test_file_too_large_2(self, client):
        url = (
            client
            + "/?status=200&content_long=test_contents_greater_than_the_max_length&no-content-length&content-type=csv"
        )
...
class TestDownload:
    ...
    def initialData(cls):
        cls.fake_context = {
            "site_url": config.get("ckan.site_url_internally")
